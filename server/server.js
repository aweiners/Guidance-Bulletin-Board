...
app.use(cors()); // In dev, allows all origins
...
app.post("/login", loginLimiter, (req, res) => {
    ...
    db.get(
        "SELECT id, username, role, password FROM accounts WHERE username = ? AND password = ?",
...
app.post("/accounts", authenticateToken, adminOnly, (req, res) => {
...
app.put("/accounts/:id", authenticateToken, adminOnly, (req, res) => {
    const { id } = req.params;
    const { username, password, role } = req.body;
    if (!username || !password || !role) {
        return res.status(400).json({ error: "All fields required" });
    ...
    db.run(
        ...
        [username.trim(), password.trim(), role.trim(), id],
        function (err) {
            if (err) return res.status(500).json({ error: err.message });
...
app.post("/forum_posts", authenticateToken, (req, res) => {
    ...
    if (!title || !post_content) {
        return res.status(400).json({ error: "Title and post content required" });
    }
    db.run(
